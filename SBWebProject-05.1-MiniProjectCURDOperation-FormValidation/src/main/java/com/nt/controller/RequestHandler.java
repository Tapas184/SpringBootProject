...
public class RequestHandler {
	...
	public String inserTingEmployee(HttpSession ses, 
			                          @ModelAttribute("emp") EmployeeModel emp, 
			                             BindingResult errors) { // BindingResult errors instead of you can take BindingException
		//for Application about some logic(Application validation)
		if(emp.getJob().equalsIgnoreCase("KING") ||emp.getJob().equalsIgnoreCase("QUEEN")||emp.getJob().equalsIgnoreCase("president")) {
			errors.rejectValue("job","emp.job.business-restriction");
			return "emp_add";
		}
		//To verify client site verification done or not
	...
	public String updateEmpEdiRequest(HttpSession ses, @ModelAttribute("emp") EmployeeModel emp, BindingResult errors) {
		...
		//for Application about some logic(Application validation)
				...
				//for checking client side verification is done or not
				if(emp.getHflag().equalsIgnoreCase("no")) 
		if (validator.supports(emp.getClass())) { // this will check the correct class or not
			validator.validate(emp, errors); // this will check whether error is available or not
			if (errors.hasErrors()) // if error is available
	...
	/*
	 * @GetMapping("/delete") public String deleteRequest(Map<String, Object> map,
	 * 
	 * @RequestParam("empno") Integer no) { //Invoke b-method String result =
	 * service.deleteEmployeeByID(no); //get Iterable Employee object
	 * Iterable<EmployeeModel> emp1 = service.getAllEmployee(); //put the result
	 * message into Map object map.put("resultMsg", result); map.put("emplist",
	 * emp1); //return Logical view name return "emp_report"; }
	 */
	@GetMapping("/delete")
